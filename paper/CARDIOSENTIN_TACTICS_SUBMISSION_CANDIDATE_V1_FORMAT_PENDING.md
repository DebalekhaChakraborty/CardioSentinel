> ## FORMAT-PENDING SUBMISSION CANDIDATE
>
> **This file is scientifically final and format-provisional.** No official
> TACTiCS 2026 call for papers, author instructions or template could be
> verified — see `audits/TACTICS_2026_SUBMISSION_REQUIREMENTS_V1.md`, which
> records three independent searches returning no authoritative source and
> lists twenty-two requirements as `NOT SPECIFIED`.
>
> **No template was fabricated.** Layout, column format, fonts, reference
> style, page limit and figure sizing are all unresolved and none has been
> guessed. This document therefore carries the manuscript content in plain
> Markdown and must be re-laid-out once the official template exists.
>
> **Author metadata is placeholder and must be completed by a human.**

# AUTHOR METADATA — TO BE COMPLETED BY A HUMAN

| Field | Value |
|---|---|
| Authors and order | `[TO BE SUPPLIED]` |
| Affiliations | `[TO BE SUPPLIED]` |
| Corresponding author and email | `[TO BE SUPPLIED]` |
| Anonymised or named submission | `[UNKNOWN — depends on the unverified CFP]` |
| Acknowledgements | `[TO BE SUPPLIED]` |
| Funding | `[TO BE SUPPLIED]` |
| Conflicts of interest | `[TO BE SUPPLIED]` |
| Organisational / client approval to publish | `[TO BE SUPPLIED]` |
| AI-use disclosure | `[TO BE SUPPLIED — requirement unverified]` |
| Originality / plagiarism declaration | `[TO BE SUPPLIED — requirement unverified]` |

**None of the above was invented.** Every field is an explicit placeholder.

---

# CardioSentinel: One Authority, Two Surfaces — Governing Experimental Evidence and Runtime Claims in an Intelligent Physical Monitoring System

## Abstract

Continuous physiological monitoring is a physical-digital systems problem, not
only a classification problem. A signal arrives from a body under uncontrolled
conditions, computation must run causally, patient-relative state accumulates
over hours, and an irrevocable decision is reported to someone who will act on
it. Systems adapting across that loop face two governance questions usually
asked separately: what evidence a system may create and use, and what its
runtime and agentic surfaces may state.

CardioSentinel is a causal ambulatory-ECG monitoring system built around both.
Learned representation, physiology, patient-relative memory under a
contamination-safe update gate, calibration and temporal episode reasoning feed
an evidence graph that grounds every generated explanation. We couple the two
questions: the provenance and authority model governing partition access, one-
shot attempt consumption and retained experimental evidence is the same model
that constrains what the runtime may assert. Prior work covers each half; to
the best of our targeted review, no prior system spans both under one
authority.

Episode reasoning improves episode-level agreement over a memoryless window
rule, on identical rows, at the promoted operating point (+0.1921, 95% CI
[0.0505, 0.3455], 12 held-out subjects). In the single evaluated generative
context, an explanation scoring 1.000 evidence fidelity, zero claim violations
and 1.000 completeness asserted that gates G1-G6 had passed while G4 and G5
were blocked; a categorical state-alignment gate refused it and the runtime
served a deterministic fallback. Replay runs at roughly 61x real time in laptop
simulation. Prespecified gates also rejected an uncertainty router and closed a
representation-improvement branch.

Evaluation uses one primary ambulatory corpus with no independent external
cohort; a single-use, encoder-only sealed evaluation showed weak, heterogeneous
cross-subject performance. This is governed monitoring research, not diagnostic
validation.

**Keywords** — Intelligent Physical Systems; physiological monitoring; evidence
governance; provenance; causal streaming; temporal reasoning; agentic AI;
trustworthy AI
...
evidence also constrains what the runtime and agentic surfaces may assert.
...
**[0.0505, 0.3455]** over 12 held-out subjects. The temporal-arm selection
that precedes it is reported as selection under a preregistered rule, not as
superiority: its paired interval **[−0.015229, 0.148951]** includes zero.
...
defect.** Running `find_violations` over the programme's
governance handbook reports **eight occurrences across three forbidden claims,
and none is an overclaim** — six are boundary statements the handbook makes in
order to prohibit what they name, and two are the word *diagnostic* used in its
statistical sense. The document that defines a boundary must state the
boundary. The exemption is
...
programme's governance handbook it reports **eight occurrences, six of them
boundary statements and two lexical collisions with the statistical sense of
*diagnostic***; run over §4's source draft, **eight, every one a quotation**. That is why
